...
{
    huidu_get_value();
    // 如果完全没有线 (8路全黑) - 保持直行，使用基准速度（不再用 min_speed 导致速度降为零）
    ...
    {
        // 方向已在 motor_init 中设置，此处只调速度
        target_speed_1 = target_speed_5[2];  // 200 mm/s
        target_speed_2 = target_speed_5[2];
    }
    // 如果全部检测到线 (8路全白) - 到达终点，停车
    ...
    // 只有中间检测到线 - 直行，使用基准速度（不再用 min_speed 导致速度只降不升）
    ...
    {
        // 方向已在 motor_init 中设置，此处只调速度
        target_speed_1 = target_speed_5[2];  // 200 mm/s 基准直行速度
        target_speed_2 = target_speed_5[2];
    }
    // 线偏左 - 左转 (S1,S2检测到线)
